...
(function () {
    ...
    function ensureModal() {
        ...
        modalEl.innerHTML = '<div class="flex items-center justify-between border-b border-cp-border p-sp-sm">'
            + '<p class="text-fs-sm font-semibold text-cp-text">Medya Kütüphanesi</p>'
            + '<button type="button" data-media-picker-close class="text-fs-sm text-cp-steel hover:text-cp-text">Kapat</button>'
            + '</div>'
    ...
    function uploadFromPicker(file, input) {
        ...
        setUploadStatus('Yükleniyor…', false);
        if (input) {
            input.disabled = true;
        }
        ...
        fetch('/admin/media/upload', { method: 'POST', body: formData, headers: { Accept: 'application/json' } })
            ...
            .then((asset) => {
                // Yeni yüklenen görseli doğrudan seç — ekstra tıklama gerekmez.
                if (asset && asset.id) {
                    selectAsset({
                        id: String(asset.id),
                        url: asset.url,
            ...
            .catch((error) => {
                setUploadStatus(error.message || 'Yükleme başarısız.', true);
            })
            .finally(() => {
                if (input) {
    ...
    function loadBody(query) {
        ...
        body.innerHTML = '<p class="p-sp-md text-center text-fs-sm text-cp-steel">Yükleniyor…</p>';
        ...
        fetch('/admin/media/picker?q=' + encodeURIComponent(query || ''))
            ...
            .catch(() => {
                body.innerHTML = '<p class="p-sp-md text-center text-fs-sm text-danger-600">Medya listesi yüklenemedi.</p>';
            });
    }
    ...
    // "Görsel Seç" tetikleyici butonları (öne çıkan görsel, SEO og_image vb.)
    ...
    // Medya kütüphanesi sayfasındaki doğrudan yükleme input'u (tam sayfa).
    document.addEventListener('change', (event) => {
        ...
        fetch('/admin/media/upload', { method: 'POST', body: formData, headers: { Accept: 'application/json' } })
            ...
            .catch(() => {
                input.value = '';
                window.alert('Yükleme başarısız.');
            });
    });
})();
